...
const DEFAULT_WORDS = ['AGENTGRID', 'X402 PROTOCOL', 'ALGORAND GRID'];
...
const styles = `
.split-flap-text{font-family:'JetBrains Mono','SFMono-Regular','Roboto Mono','Cascadia Code','Liberation Mono',Menlo,monospace;font-size:var(--split-flap-font-size,42px);font-weight:700;line-height:1;letter-spacing:.04em;font-variant-numeric:tabular-nums}
...
const toCssUnit = (value: string | number) => (typeof value === 'number' ? `${value}px` : value);
...
export const SplitFlapText: React.FC<SplitFlapTextProps> = ({
  words = ['AGENTGRID', 'X402 PROTOCOL', 'ALGORAND GRID'],
  text,
  flipDuration = 0.12,
  stagger = 0.06,
  cycleDelay = 2400,
  ...
  return (
    <>
      <style>{styles}</style>
      <div
        className={`split-flap-text inline-flex items-center justify-center whitespace-pre select-none ${className}`.trim()}
