    ...
    if (cellHash < uDensity) {
      ...
      if (toIntersection > 0.0) {
        ...
        if (dist < flakeSize) {
          ...
          float alpha = clamp(pow(intensity, uGamma), 0.0, 1.0);
...
export default function PixelSnow({
  color = '#ffffff',
  flakeSize = 0.032, // Slightly increased for bold pixel aesthetic
  minFlakeSize = 1.6, // Slightly increased
  pixelResolution = 200,
  speed = 1.25,
  depthFade = 8,
  farPlane = 20,
  ...
  return (
    <div
      ref={containerRef}
      className={`absolute inset-0 w-full h-full transform-gpu will-change-transform backface-hidden pointer-events-none ${className}`}
      style={style}
    />
  );
}
