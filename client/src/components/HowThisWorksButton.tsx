    ...
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fadeIn">
          <div 
            className="relative w-full max-w-2xl bg-black border border-white/[0.12] rounded-2xl p-6 sm:p-7 space-y-6 shadow-2xl overflow-y-auto max-h-[90vh] text-left font-mono"
          ...
          >
            ...
            <div className="flex items-start justify-between border-b border-white/[0.08] pb-4">
              ...
              <button
                ...
                className="p-2 rounded-xl bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.08] text-grid-300 hover:text-white transition-all cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            {/* Overview Paragraph */}
            <div className="text-xs sm:text-sm text-grid-200 font-sans leading-relaxed bg-white/[0.02] p-4 rounded-xl border border-white/[0.06]">
              {guide.overview}
            </div>

            {/* 3 Actionable Steps */}
            <div className="space-y-3">
              <div className="text-xs uppercase tracking-wider text-white font-bold flex items-center space-x-1.5">
              ...
              <div className="space-y-2.5">
                {guide.steps.map((step, idx) => (
                  <div 
                    key={idx}
                    className="p-3.5 rounded-xl bg-grid-950 border border-white/[0.08] flex items-start space-x-3 text-xs"
                  >
                    <div className="w-5 h-5 rounded-full bg-brand-emerald text-black font-bold text-[11px] flex items-center justify-center shrink-0 mt-0.5">
                      {idx + 1}
                    </div>
                    <div className="space-y-1 font-sans">
                      <div className="font-bold text-white font-mono text-xs flex items-center justify-between">
              ...
              <div className="space-y-2 pt-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-sans">
                  {guide.whatToLookFor.map((item, i) => (
                    <div key={i} className="flex items-start space-x-2 text-grid-300 bg-white/[0.02] p-2.5 rounded-lg border border-white/[0.04]">
            ...
            <div className="pt-3 border-t border-white/[0.08] flex justify-end">
              <button
                onClick={() => setIsOpen(false)}
                className="px-6 py-2.5 rounded-xl bg-brand-emerald hover:bg-brand-emerald/90 text-black font-mono font-bold text-xs uppercase tracking-wider shadow-glow-emerald transition-all active:scale-95 cursor-pointer"
              >
                Got It, Let&apos;s Try!
              </button>
            </div>
